...
CREATE TABLE IF NOT EXISTS public.tab_analytics (
    ...
    hourly_data JSONB DEFAULT '{}', -- Track hourly patterns
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS public.user_activity (
    ...
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    device_id UUID REFERENCES public.user_devices(id) ON DELETE SET NULL,
    activity_type TEXT NOT NULL, -- 'tab_open', 'tab_close', 'theme_change', 'login', etc.
...
CREATE TABLE IF NOT EXISTS public.subscription_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    action TEXT NOT NULL, -- 'upgraded', 'downgraded', 'canceled', 'auto_suspended', 'renewed'
...
-- SELECT cron.schedule('check-subscriptions', '0 * * * *', 'SELECT check_subscription_status()');
